   ...
   <form id="leaveProcessing" action="" method="get">
      ...
      <table>
         ...
         <tr>
            ...
            <td>{{ $leaveApplication->admin }} {{ $leaveApplication->adminName }}</td>
            ...
            <td>
               @if (($today < $leaveDate) && ($leaveApplication->status !== 'Cancelled'))
                  ...
                  <a href="{{ route('approveLeave', ['adminId' => $leaveApplication->admin, 'id' => $leaveApplication->id]) }}" class="btn btn-success" >Approve</a>
                  @endif

                  @if ($leaveApplication->status !== 'Rejected')
                  <a href="{{ route('rejectLeave', ['adminId' => $leaveApplication->admin, 'id' => $leaveApplication->id]) }}" class="btn btn-warning" >Reject</a>
                  @endif
               @else
                  -
               @endif
